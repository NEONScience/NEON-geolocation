...
test_that("Select bird named locations are correct", {
  df<-data.frame(uid=1:4,
                 namedLocation=c('SCBI_023.basePlot.brd', 'BART_003.birdGrid.brd', 
...
test_that("Select smammal named locations are correct", {
  df<-data.frame(uid=1:4,
                 namedLocation=c('BART_001.mammalGrid.mam', 'BART_001.mammalGrid.mam',
...
test_that("Select plant pres named locations are correct", {
  df<-data.frame(uid=1:3,
                 namedLocation=c('BART_036.basePlot.div', 'BART_071.basePlot.div',
...
test_that("Select soil locations are correct", {
  df<-data.frame(uid=1:2,
                 namedLocation=c('UNDE_017.basePlot.bgc', 'UNDE_013.basePlot.bgc'), 
...
test_that("Select litter trap named locations are correct", {
  df<-data.frame(uid=1:3,
                 namedLocation=c('WREF_088.basePlot.ltr', 'WREF_088.basePlot.ltr',
...
test_that("Select phenology named locations are correct", {
  df<-data.frame(uid=1:3,
                 namedLocation=c('CPER_091.phenology.phe', 'CPER_077.phenology.phe',
...
test_that("Select herb clip named locations are correct", {
  df<-data.frame(uid=1:3,
                 namedLocation=c('KONZ_045.basePlot.hbp', 'KONZ_069.basePlot.hbp',
...
test_that("Select herbaceous cfc named locations are correct", {
  df<-data.frame(uid=1:3,
                 namedLocation=c('WOOD_044.basePlot.cfc', 'SJER_012.basePlot.cfc',
...
test_that("Select root sampling locations are correct", {
  df<-data.frame(uid=1:3,
                 namedLocation=c('TALL_051.basePlot.bbc', 'TALL_051.basePlot.bbc',
...
test_that("Select woody veg mapped individuals are correct", {
  df<-data.frame(uid=1:3,
                 namedLocation=c('NIWO_061.basePlot.vst', 'NIWO_064.basePlot.vst',
...
test_that('Select beetle locations are correct', {
  df <- data.frame(uid=1:2,
                   namedLocation=c('TALL_007.basePlot.bet','TALL_013.basePlot.bet'),
                   trapID=c('N','S'))
...
test_that('Select DHP locations are correct', {
  df <- data.frame(uid=1:2,
                   namedLocation=c('SJER_055.basePlot.dhp','SJER_057.basePlot.dhp'),
                   pointID=c('W10','S2'))
...
test_that('Select CDW tally locations are correct', {
  df <- data.frame(uid=1:3,
                   namedLocation=c('MLBS_018.basePlot.cdw','GUAN_042.basePlot.cdw',
